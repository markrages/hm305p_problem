...
"""A Pythonic wrapper for the LabJack U3 driver.

  - Holds global references to Labjack devices, allowing multiple
    scripts to multiplex one interface.

  - Reads and writes digital pins by name

  - Reads and writes ADCs and DACs by voltage directly

  - Handles calibrations automatically and transparently

  - UART interface with pySerial-derived API.

  - Simplified, higher-level I2C and PWM interfaces.

Use the function u3() to get a reference to a U3 device.

See the function unit_tests() for example usage.
...
try:
    ...
    print("pip install LabJackPython")
    raise

class ClockStuckException(Exception):
    pass
...
class FIOPin(object):
    ...
    def read(self):
        self._digital()
        return bool(self.parent.getFIOState(self.number))
